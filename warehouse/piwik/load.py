...
def update(session):
    ...
    datetime = datetime.datetime.combine(date, datetime.time())
    ...
    session.query(Action).filter(Action.datetime == datetime).delete()
    session.commit()
...
def reify_visit(v):
    ...
    for action in v['actionDetails']:
        ...
        datetime = datetime.datetime.combine(visit.serverDateTime, time)
        yield Action(
            visit_id = int(v['idVisit']),

            page_id = int(action['pageId']),
            page_id_action = int(action['pageIdAction']),
